...
def is_end_pos(map_rows, pos, goal_num):
    x, y = pos
    if goal_num % 2 == 1: return x == 0 and y == -1
...
while True:
    ...
    finished = False
    for goal_num, pos in elf_positions:
        if is_valid_pos(new_map_rows, pos) or is_end_pos(new_map_rows, pos, goal_num - 1):
            new_positions.add((goal_num, pos))
        for n in range(4):
            n_pos = get_neighbor(new_map_rows, pos, n, False)
            
            if is_end_pos(new_map_rows, n_pos, goal_num):
                if goal_num == 2: 
                    finished = True
                else: 
                    new_positions.add((goal_num + 1, n_pos))
            elif is_valid_pos(new_map_rows, n_pos):
                new_positions.add((goal_num, n_pos))
