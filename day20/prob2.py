...
def wrap_offset(offset, size):
    if offset > 0: return offset % (size - 1)
    else: return -((-offset) % (size - 1))
...
for i, num in enumerate(orig_nums): 
    ...
    nums_with_indices.append((i, num * 811589153))
...
for mix in range(10):
    ...
    for i in range(len(orig_num_indices)):
        ...
        offset = wrap_offset(num, nums_count)
